        ...
        <div class="event-cards">
            {#each upcoming as ev (ev.id)}
                <div class="event-card">
                    ...
                    <button class="btn-cancel" onclick={() => cancelEnrollment(ev)} title="Odhlásit se z termínu">✕</button>
                </div>
            {/each}
        </div>
    {/if}
...
<style>
    ...
    .count { background: #9b2ec8; color: #fff; font-size: .72rem; font-weight: 700; padding: .1rem .5rem; border-radius: 100px; }
    ...
    .ec-course { display: block; font-weight: 700; font-size: .95rem; color: #1a0030; text-decoration: none; margin-bottom: .2rem; }
    ...
    .ec-badge-past { font-size: .72rem; font-weight: 600; padding: .15rem .5rem; background: #e8e8e8; color: #666; border-radius: 100px; }
